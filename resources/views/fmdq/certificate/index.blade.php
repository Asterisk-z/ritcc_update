    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="content-page-header">
                ...
                <button type="button" class="btn btn-primary mt-1" data-bs-toggle="modal" data-bs-target="#standard-modal"><i class="fa fa-plus-circle me-2" aria-hidden="true"></i>Add Certificate</button>
                {{-- modal --}}
                <div id="standard-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('certificate.mgt.create') }}" method="POST" id="myForm" class="needs-validation" novalidate>
                                @csrf
                                    ...
                                    <div class="form-row row">
                                        ...
                                        <div class="col-md-12 mb-3">
                                            ...
                                            <select name="auctioneer" id="validationCustom01" class="form-control" required>
                                                ...
                                                <option value="{{ $auctioneer->id }}">{{ $auctioneer->email ." | ".$auctioneer->user_inst->institutionName }}</option>
                                                @empty
                                                @endforelse
                                            </select>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select name="securityType" id="validationCustom01" class="form-control" required>
                                                <option value="">--Select--</option>
                                                @forelse ($securityTypes as $securityType)
                                                <option value="{{ $securityType->securityTypeCode }}">{{ $securityType->description  }}</option>
                                                @empty
                                                @endforelse
                                            </select>
                                            <div class="invalid-feedback">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="text-center">
                                                            <h6>Security Code : <strong>{{ $security->securityCode }}</strong></h6>
                                                            <br>
                                                            <h6>Offer Amount : <strong>{{ $security->offerAmount }}</strong></h6>
                                                            <br>
                                                            <h6>Auctioneer Email : <strong>{{ $security->auctioneer->email }}</strong></h6>
                                                            <br>
                                                            <h6>Institution : <strong>{{ $security->auctioneer->user_inst->institutionName }}</strong></h6>
                                                            <br>
                                                            <h6>Security ISIN NUmber : <strong>{{ $security->isinNumber }}</strong></h6>
                                                            <br>
                                                            <h6>Issuer Number : <strong>{{ $security->issueCode }}</strong></h6>
                                                            <br>
                                                            <h6>Issue Date : <strong>{{ date('F d, Y h:m:s',strtotime($security->issueDate)) }}</strong></h6>
                                                            <br>
                                                            <h6>Inputter: <strong>{{ $security->createdBy }}</strong></h6>
                                                            <br>
                                                            <h6>Created Date: <strong>{{ date('F d, Y h:m:s',strtotime($security->createdDate))}}</strong></h6>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer">
                                        ...
                                        <div id="edit{{ $security->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route('certificate.mgt.update') }}" method="POST" class="needs-validation" id="update" novalidate>
                                                        @csrf
                                                            ...
                                                            <div class="form-row row">
                                                                ...
                                                                <div class="col-md-12 mb-3">
                                                                    ...
                                                                    <select name="auctioneer" id="validationCustom01" class="form-control" required>
                                                                        <option value="">--Select--</option>
                                                                        @forelse ($auctioneers as $auctioneer)
                                                                        <option value="{{ $auctioneer->id }}" {{ ($security->auctioneerRef == $auctioneer->id) ? 'selected' : '' }}>{{ $auctioneer->email ." | ".$auctioneer->user_inst->institutionName }}</option>
                                                                        @empty
                                                                        @endforelse
                                                                    </select>
                                                                    <div class="invalid-feedback">
                                                                ...
                                                                <div class="col-md-6 mb-3">
                                                                    ...
                                                                    <select name="securityType" id="validationCustom01" class="form-control" required>
                                                                        <option value="">--Select--</option>
                                                                        @forelse ($securityTypes as $securityType)
                                                                        <option value="{{ $securityType->securityTypeCode }}" {{ ($security->securityType == $securityType->securityTypeCode) ? 'selected' : '' }}>{{ $securityType->description  }}</option>
                                                                        @empty
                                                                        @endforelse
                                                                    </select>
                                                                    <div class="invalid-feedback">
                                        ...
                                        <div id="delete{{ $security->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route('certificate.mgt.delete') }}" method="POST" class="needs-validation" id="myForm" novalidate>
                                                        @csrf
                                                        <input type='hidden' name='security_ref' value="{{ $security->id }}" />
                                                        <div class="modal-body">
